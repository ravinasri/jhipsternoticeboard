...
export class AuthorService {
  ...
  protected convertDateFromClient(author: IAuthor): IAuthor {
    const copy: IAuthor = Object.assign({}, author, {
      creationdate: author.creationdate && author.creationdate.isValid() ? author.creationdate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((author: IAuthor) => {
        author.creationdate = author.creationdate ? moment(author.creationdate) : undefined;
      });
    }
    return res;
  }
